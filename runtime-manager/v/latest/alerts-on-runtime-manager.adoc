...
Anypoint Platform enforces rate limits that determine how often an alert is triggered. These limits prevent excessive emails from being sent. Each alert is triggered at most once every five seconds. This means that only one alert notification email is sent every 5 seconds. This limit is per the combination of Organization, Environment, Alert (Name/ID) and Resource (e.g.: Application(s), Server, Cluster or Group).
