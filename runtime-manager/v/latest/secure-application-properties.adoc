...
1. Identify the property names that you wish to safely hide by listing them as a comma separated array as the value of the `secure.properties` key inside the Mule application's `mule-artifact.json` file. In the example below, there are four total properties used in the application (lines 1-4 below), which have been defined in the mule-app.properties file with values for local testing (optional). Line 5 flags two of the properties as secure. 

.mule-artifact.json
[source,json, linenums]
----
1. "secure.properties": ["db1.password", "birthdate", "ssn"]
----

[NOTE]
===
In Mule 3, you identify the property names that you wish to safely hide by listing them as a comma separated list as the value of the `secure.properties` key inside the Mule application's `mule-app.properties` file.
...
[NOTE] 
====
The `secure.properties` entry *can only* be written inside `mule-app.properties`, but affects all properties no matter where (or if) they are defined. In general, you should store application properties in a separate Properties Placeholder file.
====
