...
1. Identify the property names that you wish to safely hide by listing them as a comma separated list as the value of the `secure.properties` key inside the Mule application's `mule-app.properties` file. In the example below, there are four total properties used in the application (lines 1-4 below), which have been defined in the mule-app.properties file with values for local testing (optional). Line 5 flags two of the properties as secure. 
