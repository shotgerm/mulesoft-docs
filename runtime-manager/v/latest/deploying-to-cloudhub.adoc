...
If you also have any property names set in a mule-app.properties file inside your application or in bundled configuration property files, then when your application is deployed, any entries in the CloudHub Properties tab with the same name will override the matching value bundled with the application.

[Note]
It is possible to change the behavior of the application to not allow CloudHub properties to override properties bundled with the deployable archive. You do this by changing the options in the Configuration Property element in the Mule application to prefer configuration property files over JVM system environment variables. See link:/mule-user-guide/v/4.1/configuring-properties[Documentation on Configuration Property] for more information on non-default configuration properties options.
