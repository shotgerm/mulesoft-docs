...
What if I want to define a secure property to depend on a previously defined secure property?::
  It is possible to define a property to depend on a previously defined one, just using the same syntax: `property=My dependent ${secure::dependent.property} value`. **Note**: Secure Configuration Properties can depend on other Secure Configuration Properties. Not on Configuration Properties.
