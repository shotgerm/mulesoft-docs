...
The `mule-maven-plugin` allows you to deploy Mule applications to different kinds of servers: Standalone (both Community and Enterprise), clustered, Anypoint Runtime Manager and CloudHub. It is part of the framework for developing Mule applications with Maven, an overview of which you can find in link:/mule-user-guide/v/3.9/using-maven-with-mule[Using Maven with Mule]. The most important capabilities provided by the plugin are running integration tests, and deploying applications to different environments.
...
NOTE: Only platform users are allowed in this configuration. It's not possible to user SSO/Federated as username.
...
<plugin>
    ...
    <configuration>
        ...
        <environment>Production</environment>
        <properties> 
          <!-- All propertied that users define in mule-app.properties as maven won't load properties file -->
